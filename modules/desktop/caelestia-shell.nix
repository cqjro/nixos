...
# Determine mode from stylix polarity
...
in
{
	...
	# Drop the scheme file where caelestia-cli expects user schemes
  home.file.".local/state/caelestia/schemes/stylix/default/${mode}.txt" = {
    source = caelestiaScheme;
  };

  # Set the active scheme — caelestia watches this file reactively
  home.file.".local/state/caelestia/scheme.json" = {
    ...
    # Prevent home-manager from erroring if caelestia has already written here
    force = true;
  };
	...
	programs.caelestia = {
		...
		settings = {
			bar = {
				activeWindow.rotateCounterClockwise = true;
